...
```{r set-seed}
set.seed(1993)                 # make sure to change the seed
```
...
sam <- subset(samp,select = c("area","price","PID"))
sam2 <- sam[order(sam$price),]
barplot(height = sam2$price,xlab = "size",ylab = "price")
sst<-summarise(sam2,meanp=mean(sam2$area),irqp=IQR(sam2$area),sdp=sd(sam2$area),madp=mad(sam2$area),medianp=median(sam2$area),q1=quantile(sam2$area,0.25),q3=quantile(sam2$area,0.75))
sst
```
...
summarise(ames,mu = mean(ames$area))
```
